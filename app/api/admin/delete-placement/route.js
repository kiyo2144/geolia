...
export async function POST(request) {
  // ここで想定外の例外が起きると、Next.jsの既定の500エラーページ（HTML）が
  // 返ってしまい、呼び出し側のres.json()が失敗して原因不明の「通信に失敗しました」
  // としか表示されなくなる。必ずJSONでエラー内容を返せるよう全体をtry/catchする。
  try {
    ...
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
      return NextResponse.json({ error: "サーバー側でSUPABASE_SERVICE_ROLE_KEYが未設定です" }, { status: 500 });
    }
    ...
    console.error("AR配置の削除中に予期しないエラーが発生しました:", error);
    return NextResponse.json({ error: "サーバー側で予期しないエラーが発生しました" }, { status: 500 });
  }
}
