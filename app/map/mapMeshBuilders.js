// 3Dマップのメッシュ生成に関する共通ロジック。
// GLBエクスポート機能（exportGlb.js）と、マップ上の任意地点を一人称視点で
// 確認する機能（FirstPersonView.js）の両方から利用する。

import * as THREE from "three";

// OSM建物の色分け（app/map/MapView.js の BUILDING_COLOR / NAMED_BUILDING_COLOR と揃えること）
export const BUILDING_COLOR = "#c9b8a3";
export const NAMED_BUILDING_COLOR = "#e0a526";

export function lngLatToTileFloat(lng, lat, z) {
  const n = 2 ** z;
...
export function makeProjector(bbox) {
  ...
  project.unproject = (x, y) => ({
    lng: lng0 + x / metersPerDegLng,
...
export async function buildBasemapTexture(bbox, basemapKey) {
...
export function buildParcelGroup(name, features, colorField, project, sampleElevation, minElevation) {
